...
# 커스텀 경로에 생성
pnpm migration:create:custom -- src/common/database/migrations/my-migration
```
...
# 커스텀 경로에 생성
pnpm migration:generate:custom -- src/common/database/migrations/my-migration
```
...
```bash
# 스테이징 환경에서 테스트
pnpm migration:show  # 적용할 마이그레이션 확인
pnpm migration:run   # 마이그레이션 실행
pnpm migration:show  # 적용 결과 확인
```
...
1. 에러 메시지 확인
2. 마이그레이션 상태 확인: `pnpm migration:show`
3. 필요시 수동으로 DB 수정 후 fake 실행: `pnpm migration:run:fake`
