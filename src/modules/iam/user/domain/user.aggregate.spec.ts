...
describe('User Aggregate', () => {
  describe('create', () => {
    it('유효한 파라미터로 User를 생성해야 한다', () => {
      const username = UserName.from('test@example.com');
      const password = UserPassword.from('password123');
  ...
  describe('from', () => {
    it('기존 데이터로부터 User를 재구성해야 한다', () => {
      ...
      const username = UserName.from('existing@example.com');
      const password = UserPassword.from('existingpassword');
